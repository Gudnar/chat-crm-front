  ...
  <div>
    ...
    <div v-else style="display:flex; flex-direction:column; gap:10px; margin-top:16px;">
      <div
        ...
        class="ide-ia-card"
        :style="{ border: '1px solid ' + (h.activa ? h.color+'33' : 'var(--border-card)') }"
      >
        <div style="display:flex; align-items:flex-start; gap:12px;">
          <div class="hc-ico" :style="{ background: h.color+'22' }">🔧</div>
          <div style="flex:1;">
            <div style="display:flex; align-items:center; gap:8px; margin-bottom:3px;">
              <span style="font-size:13px; font-weight:700; color:var(--text-primary);">{{ h.label }}</span>
              ...
              <span v-else class="hc-badge" style="background:color-mix(in srgb, var(--text-disabled) 13%, transparent);color:var(--text-faint);">Inactiva</span>
            </div>
            <div style="font-size:11px; color:var(--text-faint);">{{ h.descripcion }}</div>
          </div>
        ...
        <div v-if="h.activa" style="display:flex; align-items:center; gap:8px; margin-top:10px;">
          <div style="font-size:11px; color:var(--text-faint); flex-shrink:0;">Confianza:</div>
      ...
      <div class="hc-add" @click="dialogAdd = true">
        ...
        <span style="font-size:12px; color:var(--text-disabled);">Agregar herramienta</span>
      </div>
    </div>

    <!-- Dialog -->
    <v-dialog v-model="dialogAdd" max-width="480" persistent>
      <v-card style="background:var(--bg-surface); border:1px solid var(--border); border-radius:12px;">
        <v-card-title style="color:var(--text-heading); font-size:15px; font-weight:700; padding:20px 24px 12px;">Nueva Herramienta</v-card-title>
        ...
        <v-card-actions style="padding:0 24px 20px; justify-content:flex-end; gap:8px;">
          <v-btn text @click="dialogAdd = false" style="color:var(--text-faint); font-size:12px;">Cancelar</v-btn>
  ...
  computed: {
    stats() {
      return [
        ...
        { label: 'Inactivas',         value: this.herramientas.filter(h => !h.activa).length,                       color: 'var(--text-disabled)' },
      ];
    },
  },
  async mounted() {
...
.hc-param { font-size: 11px; background: var(--bg-page); color: var(--text-muted); padding: 3px 8px; border-radius: 4px; border: 1px solid var(--border-card); }
.hc-add   { display: flex; align-items: center; justify-content: center; gap: 8px; border: 1px dashed var(--border); cursor: pointer; padding: 14px; border-radius: 12px; color: var(--text-disabled); transition: border-color 0.15s; }
