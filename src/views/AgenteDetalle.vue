<template>
  <div class="ide-mod">
    ...
    <div class="ide-subnav">
      ...
      <div style="margin-top:auto; padding-top:12px; border-top:1px solid var(--border-card);">
        <button class="ide-sn-btn" @click="$router.push({ name: 'agentes' })" style="color:var(--text-disabled);">
    ...
    <div v-else-if="agente && seccion !== 'probar'" class="ide-body">
        ...
        <div style="max-width:680px;">
          <div class="ide-ia-card" style="margin-bottom:16px;">
            <div class="ad-form-grid">
              <div class="ide-field">
                ...
                <div style="font-size:11px; color:var(--text-faint); margin-top:2px;">{{ formConfig.maxTokens }} tokens ≈ {{ Math.round(formConfig.maxTokens * 0.75) }} palabras</div>
              </div>
              <div class="ide-field">
          ...
          <div class="ide-field" style="margin-bottom:16px;">
            <label>
              ...
              <span style="margin-left:8px; font-size:10px; font-weight:500; color:var(--text-disabled);">Define completamente el comportamiento del agente</span>
            </label>
            ...
            <div style="display:flex; justify-content:space-between; margin-top:4px;">
              <span style="font-size:10px; color:var(--text-disabled);">{{ (formConfig.systemPrompt || '').length }} caracteres</span>
              <span style="font-size:10px; color:var(--text-faint);">{{ agente.nombre }} · {{ formConfig.modelo }}</span>
            </div>
          </div>

          <div class="ide-ia-card" style="margin-bottom:16px;">
            <div style="font-size:12px; font-weight:700; color:var(--text-primary); margin-bottom:10px;">Plantillas rápidas</div>
      ...
      <template v-else-if="seccion === 'calificacion'">
        ...
        <div style="max-width:620px;">
          <div class="ide-ia-card" style="margin-bottom:16px;">
            <div style="font-size:13px; font-weight:700; color:var(--text-primary); margin-bottom:14px;">Umbrales de clasificación</div>
            <div style="display:flex; flex-direction:column; gap:12px;">
              <div v-for="tier in tiers" :key="tier.id" class="ad-tier-row">
                <div class="ad-tier-dot" :style="{ background: tier.color }"></div>
                <span style="font-size:13px; color:var(--text-primary); flex:1;">{{ tier.label }}</span>
          ...
          <div class="ide-ia-card" style="margin-bottom:16px;">
            <div style="font-size:13px; font-weight:700; color:var(--text-primary); margin-bottom:14px;">Criterios de calificación</div>
            <div style="display:flex; flex-direction:column; gap:10px;">
              <div v-for="c in criterios" :key="c.id" style="display:flex; align-items:center; justify-content:space-between; padding:10px 12px; background:var(--bg-page); border-radius:8px; border:1px solid var(--border-card);">
                <div>
                  <div style="font-size:12px; font-weight:700; color:var(--text-primary);">{{ c.label }}</div>
                  <div style="font-size:11px; color:var(--text-faint); margin-top:2px;">{{ c.desc }}</div>
                </div>
                <div style="display:flex; align-items:center; gap:8px;">
      ...
      <template v-else-if="seccion === 'escalado'">
        ...
        <div style="max-width:620px;">
          <div class="ide-ia-card" style="margin-bottom:16px;">
            <div style="font-size:13px; font-weight:700; color:var(--text-primary); margin-bottom:14px;">Reglas de escalado automático</div>
            <div style="display:flex; flex-direction:column; gap:10px;">
              <div v-for="r in reglas" :key="r.id" style="display:flex; align-items:flex-start; justify-content:space-between; padding:12px 14px; background:var(--bg-page); border-radius:8px; border:1px solid var(--border-card);">
                <div style="flex:1; padding-right:16px;">
                  <div style="font-size:12px; font-weight:700; color:var(--text-primary);">{{ r.label }}</div>
                  <div style="font-size:11px; color:var(--text-faint); margin-top:2px; line-height:1.5;">{{ r.desc }}</div>
                </div>
          ...
          <div class="ide-ia-card" style="margin-bottom:16px;">
            <div style="font-size:13px; font-weight:700; color:var(--text-primary); margin-bottom:10px;">Mensaje de transferencia</div>
...
.ad-tier-row { display: flex; align-items: center; gap: 12px; padding: 10px 12px; background: var(--bg-page); border-radius: 8px; border: 1px solid var(--border-card); }
